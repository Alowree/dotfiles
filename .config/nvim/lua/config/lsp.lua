...
local function on_list(list)
	...
	require("config.picker").open(list.items, {
		title = list.title,
		format = function(item)
			return ("%s:%d: %s"):format(vim.fn.fnamemodify(filename(item), ":."), item.lnum, vim.trim(item.text or ""))
		end,
		preview = function(item, ctx)
			ctx.set_file(filename(item), item.lnum, item.col)
		end,
		on_confirm = function(item)
			vim.cmd.edit(vim.fn.fnameescape(filename(item)))
...
vim.api.nvim_create_autocmd("BufWritePre", {
	...
	callback = function(args)
		...
		-- Only format when a formatter is actually attached; stay silent otherwise.
